module Validations
  class PhoneValidator < ActiveModel::Validator
    ...
    PHONE_REGEXP = Regexp.union(CN_PHONE_REGEXP, JP_PHONE_REGEXP)

    def validate(record)
      value = record.verifiedMobile
      return record.errors.add(:verifiedMobile, :blank) if value.nil?
      record.errors.add(:verifiedMobile, :invalid) unless value.to_s.match?(PHONE_REGEXP) || value.to_s.match?(FAKE_PHONE_REGEXP)
    end
  end
end